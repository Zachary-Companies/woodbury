...
const DEFAULT_SKILLS: SkillDescriptor[] = [
  ...
  },
  {
    ...
    promptGuidance: 'Call the intelligence generation tools with the approved design contract and tight constraints. Require a real saved composition result; do not substitute one-off execution. The generate_pipeline tool defaults to v2 format (file-backed TypeScript pipeline directory) — each script node becomes a .ts file. Pass format: "v1" only if legacy inline format is explicitly needed. For v2, the result includes pipelineDir, scriptFiles, and a manifest. Prefer __variable__ nodes for shared pipeline-form inputs: if the same semantic input is needed by multiple nodes, expose it once with variableNode.exposeAsInput=true, set inputName/description/required/generationPrompt as needed, and connect that variable value downstream.',
  ...
  },
  {
    ...
    promptGuidance: 'Inspect the generated composition, parse-check embedded code, verify wiring, and reject malformed code blobs. For v2 pipelines, the .ts files are in the pipelineDir — you can read them to verify correctness. Check that each file exports execute(), has proper @input/@output annotations, and that the pipeline.json manifest correctly references each file. Normalize repeated external inputs into a single exposed __variable__ node whenever the same user value is repeated across multiple consumers. Prefer regenerating or repairing through intelligence tools over manual file edits unless the user explicitly escalates.',
    preferredSubagent: 'execute',
